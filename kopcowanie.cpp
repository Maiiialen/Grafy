...
void tworzenie(vector <int> &tab, int poczatek, int n, int i){
    ...
    if(largest != i){                                            //sprawdzenie czy wartość badana "i" jest nadal największa
        zmienna = tab[largest + poczatek];                       //zamiana wartości pod indeksami "largest" i "i"
...
void kopcowanie(vector <int> &tab, int poczatek, int koniec){
    ...
    for(int i = n - 1; i > 0; i--){                             //pętla wykonująca się dla całej tablicy
        zmienna = tab[0 + poczatek];                            //zamiana wartości
